...
MODEL_CONFIGS = {
    ...
    "azure": {
        "available_models": [
            "azure02-gpt-5",  # Current deployment
        ],
        "default_model": "azure02-gpt-5",
...
def get_llm_client(provider, model_name=None):
    ...
    # Azure OpenAI models
        ...
        # Use deployment name from env if not passed or if passed acts as override logic
        # But typically for Azure the model name in config maps to deployment name
        deployment_name = model_name if model_name else MODEL_CONFIGS["azure"]["default_model"]
        
        # If user has a specific env var for deployment, we might prefer that
        if AZURE_OPENAI_DEPLOYMENT_NAME:
            deployment_name = AZURE_OPENAI_DEPLOYMENT_NAME
            
        print(f"Using AzureChatOpenAI for {deployment_name}")
...
async def get_async_llm_client(provider, model_name=None):
    ...
    if provider == "openai":
        ...
        logger.info(
            f"[get_async_llm_client] Creating async AzureChatOpenAI client with deployment {deployment_name}"
        )
        return AzureChatOpenAI(
            azure_deployment=deployment_name,
            api_version=AZURE_OPENAI_API_VERSION,
        ...
        if AZURE_OPENAI_API_KEY:
            supported_providers.append("azure")
