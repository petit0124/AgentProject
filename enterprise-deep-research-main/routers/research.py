...
async def deep_research(request: ResearchRequest, background_tasks: BackgroundTasks):
    ...
    # FORCE AZURE OVERRIDE
    import os
    env_provider = os.environ.get("LLM_PROVIDER", "")
    if env_provider.lower() in ["azureopenai", "azure"]:
        logger.info(f"Enforcing Azure OpenAI provider (overriding request provider '{request.provider}')")
        request.provider = "azure"
        
        # Also enforce the model if set
        env_model = os.environ.get("LLM_MODEL")
        if env_model:
             logger.info(f"Enforcing Azure OpenAI model: {env_model} (overriding '{request.model}')")
